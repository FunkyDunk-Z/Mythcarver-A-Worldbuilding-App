...
const protect = async (req, res, next) => {
    try {
        ...
        const { iat, _id } = jwt.verify(token, env.JWT_SECRET);
        if (!iat) {
...
const forgotPassword = async (req, res, next) => {
    ...
    try {
        ...
        const resetURL = `${req.protocol}:/${req.get('host')}/api/v2/users/reset-password/${resetToken}`;
...
const updateMyAccount = async (req, res, next) => {
    try {
        ...
        const filteredBody = filterObj(req.body, 'firstName', 'lastName', 'username', 'email', 'avatarURL');
...
export { signUp, login, logout, protect, forgotPassword, resetPassword, updatePassword, getMyAccount, updateMyAccount, deleteMyAccount, isLoggedIn, };
